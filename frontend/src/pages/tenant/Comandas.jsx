...
// Preço na moeda da empresa (brl/jpy) — mesmo padrão de tenant/Agenda.jsx e
// tenant/Caixa.jsx.
...
function Comandas({ t: tProp, idioma: idiomaProp, empresa, empresaId }) {
  ...
  // abas Profissionais/Serviços do painel, como em tenant/Agenda.jsx — sem
  // catálogo fixo e sem restrição de "qual profissional faz qual serviço"
  // (mesma simplificação já adotada na Agenda).
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'servico') {
      const s = servicosLista.find(sv => sv.nome === value);
      atualizado.valor = s ? String(s.precoMinimo) : '';
    }
  ...
  const handleFecharComanda = async (item) => {
    ...
    const servicoInfo = servicosLista.find(s => s.uuid === item.servicoId);
    const sugestao = item.valor > 0 ? item.valor : (servicoInfo?.precoMinimo ?? 0);
  ...
  const handleAdicionarItem = async (e) => {
    ...
    try {
      const servicoInfo = servicosLista.find(s => s.nome === novoItem.servico);
      const profInfo = profissionaisLista.find(p => p.nome === novoItem.profissional);
    ...
    <div className="page-container">
      ...
      <section className="caixa-status">
        ...
        <div className="status-card">
          ...
          <p className="saldo-final">{t('comum.total')}: <strong>{formatarValor(totalRealizadoHoje, moeda)}</strong></p>
        </div>
      </section>
        ...
        <section className="form-section">
          ...
          <form onSubmit={handleAdicionarItem}>
            ...
            >
              ...
              {servicosLista.map((s) => (
                <option key={s.id} value={s.nome}>{s.nome} - {formatarValor(s.precoMinimo, moeda)}</option>
              ))}
            </select>
            <select
              name="profissional"
              ...
              <tbody>
                {realizadosHoje.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td>{item.servico}</td>
                    <td>{item.profissional}</td>
                    <td style={{ fontWeight: 'bold', color: '#d4af37' }}>{formatarValor(item.valor, moeda)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
