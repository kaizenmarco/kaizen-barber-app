...
function Clientes({ t: tProp, idioma: idiomaProp, empresaId }) {
  ...
  // abas Profissionais/Serviços do painel (RLS isola por empresa), assim
  // como em tenant/Agenda.jsx — sem catálogo fixo compartilhado.
  ...
  // usado no cálculo de horários livres do agendamento rápido abaixo.
  ...
  const profissionalNovoAgendamentoObj = profissionaisLista.find(p => p.nome === novoAgendamentoForm.profissional);
  const slotsDisponiveisNovoAgendamento = (!novoAgendamentoForm.data || !servicoNovoAgendamentoObj || !profissionalNovoAgendamentoObj)
    ? []
    : getSlotsLivresNoDia(
        profissionalNovoAgendamentoObj.uuid,
