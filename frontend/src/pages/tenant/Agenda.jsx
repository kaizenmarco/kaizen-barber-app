...
import { getSlotsLivresNoDia, paraMinutos, paraHHMM, getHorarioDoDiaProfissional, buscarHorarioEstendido, buscarHorariosPorProfissional, HORARIO_ESTENDIDO_PADRAO } from '../../config/horariosTenant';
...
import { contarCancelamentosUltimaHora, LIMITE_ULTIMA_HORA_MIN } from '../../config/reincidenciasTenant';

// Preço mostrado na moeda que a empresa escolheu no cadastro (brl/jpy) —
// mesmo padrão de tenant/Servicos.jsx.
...
function Agenda({ t: tProp, idioma: idiomaProp, empresa }) {
  ...
  const [abaProfissional, setAbaProfissional] = useState('');
  ...
  // Profissionais e serviços MULTI-TENANT: vêm das tabelas cadastradas nas
  // abas Profissionais/Serviços do painel (RLS isola por empresa) — não
  // existe mais um catálogo fixo compartilhado por todo mundo. Sem nenhum
  // profissional cadastrado ainda, a Agenda mostra um aviso em vez da grade.
  ...
  useEffect(() => {
    if (!abaProfissional && profissionaisLista.length > 0) {
      setAbaProfissional(profissionaisLista[0].uuid);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [profissionaisLista]);

  // Horário de trabalho de cada profissional (aba "Horário de Trabalho") —
  // busca tudo de uma vez e organiza por profissional/dia da semana; usado
  // no lugar do horário único do salão que o app original tinha fixo no código.
  const [horariosPorProfissional, setHorariosPorProfissional] = useState({});
  useEffect(() => {
    buscarHorariosPorProfissional().then(setHorariosPorProfissional);
  }, []);

  // Serviços cadastrados na aba Serviços (eh_pacote = false — pacotes não
  // entram na Agenda).
  ...
  const slotsDisponiveisNovoAgendamento = (!profissionalNovoObj || !servicoNovoObj || !novoAgendamento.data) ? [] : (() => {
    ...
    return getSlotsLivresNoDia(profissionalNovoObj.uuid, dataObj, duracaoNovoAgendamento, intervalosOcupadosNoDia, horariosPorProfissional, horarioEstendido);
  })();
  ...
  const slotsDisponiveisEdicao = (!agendamentoEditando || !edicaoForm.data) ? [] : (() => {
    ...
    return getSlotsLivresNoDia(agendamentoEditando.profissionalId, dataObj, duracaoEdicao, intervalosOcupadosEdicao, horariosPorProfissional, horarioEstendido);
  })();
  ...
  const horarioDoDiaSelecionado = getHorarioDoDiaProfissional(abaProfissional, dataDiaSelecionadoObj, horariosPorProfissional, horarioEstendido);
  ...
  const horariosPorDiaSemana = diasDaSemanaAtual.map(d => getHorarioDoDiaProfissional(abaProfissional, d, horariosPorProfissional, horarioEstendido));
  ...
  if (!carregandoProfissionais && profissionaisLista.length === 0) {
    return (
      <div className="page-container">
        <h2>{t('agendamentos.titulo')}</h2>
        <p style={{ textAlign: 'center', color: '#999' }}>
          Cadastre pelo menos um profissional (aba "Profissionais") e configure o horário de
          trabalho dele (aba "Horário de Trabalho") antes de usar a Agenda.
        </p>
      </div>
    );
  }
                        ...
                        {!compacto && (
                          <>
                            ...
                            <div className="agenda-card-rodape">
                              ...
                              <span className="agenda-card-preco">{formatarPreco(a.preco, moeda)}</span>
                            </div>
                          </>
                        )}
                      </div>
          ...
          >
            ...
            <div className="detalhe-campo-linha">
              <div className="detalhe-campo-texto">
                ...
                <span className="detalhe-campo-valor">{formatarPreco(detalhesAgendamento.preco, moeda)}</span>
              </div>
            </div>
            ...
            {carregandoProntuario ? (
              ...
              prontuarioItens.map(item => (
                <div key={item.id} className="prontuario-item">
                  ...
                  {item.preco ? <span style={{ float: 'right', color: '#d4af37' }}>{formatarPreco(item.preco, moeda)}</span> : null}
                </div>
              ))
            )}
          </div>
